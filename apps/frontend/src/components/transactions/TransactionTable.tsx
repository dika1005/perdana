...
export const TransactionTable: React.FC<TransactionTableProps> = ({
  ...
  const getPaymentBadge = (status: PaymentStatus) => {
    switch (status) {
      case 'PAID':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">✅ LUNAS</span>;
      case 'DP':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">⏳ DP</span>;
      case 'UNPAID':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-300">❌ Belum Bayar</span>;
      default:
        return null;
    }
  };
  ...
  const getOrderBadge = (status: OrderStatus) => {
    switch (status) {
      case 'ANTRIAN':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300">🕐 Antrian</span>;
      case 'PROSES':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">🔄 Proses</span>;
      case 'SELESAI':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-purple-100 text-purple-700 dark:bg-purple-950/40 dark:text-purple-300">✅ Selesai</span>;
      case 'DIAMBIL':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">📦 Diambil</span>;
      default:
        return null;
    }
  };
                      ...
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {isDP && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-amber-500/15 border border-amber-500/30 text-amber-700 dark:text-amber-300 font-bold rounded-lg hover:bg-amber-500 hover:text-white transition-all flex items-center gap-1 text-xs"
                        ...
                        <button
                          ...
                          className="px-2.5 py-1.5 skeuo-button text-text-muted hover:text-emerald-600 rounded-lg flex items-center gap-1 text-xs font-bold"
                          title="Cetak Struk"
                        >
