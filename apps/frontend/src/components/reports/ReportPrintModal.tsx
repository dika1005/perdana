...
export const ReportPrintModal: React.FC<ReportPrintModalProps> = ({
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Rekap Laporan Keuangan - ${STORE_IDENTITY.name} (${selectedYear})</title>
          <style>
  ...
  return (
    <Modal
      open={isOpen}
      onClose={onClose}
      title="Cetak Rekap Laporan Bulanan & Keuangan"
    ...
    >
      ...
      >
        ...
        <div className="text-center border-b-2 border-slate-900 pb-3 mb-4">
          <h1 className="text-lg font-black tracking-tight uppercase">{STORE_IDENTITY.name}</h1>
        ...
        <h3 className="font-bold text-xs uppercase tracking-wider text-slate-800 mt-4 mb-1.5 flex items-center gap-1.5">
          <TrendingUp className="w-3.5 h-3.5" /> Rincian Penjualan 12 Bulan (Tahun {selectedYear}):
        </h3>
        ...
        {topProducts && topProducts.length > 0 && (
          <div className="mt-4">
            <h3 className="font-bold text-xs uppercase tracking-wider text-slate-800 mb-1.5 flex items-center gap-1.5">
              <Award className="w-3.5 h-3.5" /> Top 5 Produk Paling Laris:
            </h3>
