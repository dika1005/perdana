...
export const InventoryLotModal: React.FC<InventoryLotModalProps> = ({
  ...
  return (
    <Modal
      open={isOpen}
      onClose={onClose}
      title={`Kelola Lot Roll & Offcut: ${material.name}`}
      subtitle={
        <>Stok Total: <strong className="font-mono text-text-main">{material.stock} {material.unit}</strong> • Terkunci: <strong className="font-mono text-amber-600 dark:text-amber-400">{material.reserved_stock} {material.unit}</strong></>
      }
        ...
        {showAddForm && (
          <form onSubmit={handleCreateLot} className="p-4 rounded-xl bg-white dark:bg-slate-900 border border-purple-200 dark:border-purple-900/60 space-y-3 shadow-sm">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <button type="button" onClick={() => setShowAddForm(false)} className="text-slate-400 hover:text-slate-600 text-xs cursor-pointer">Batal</button>
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-3.5 py-1.5 text-xs font-bold rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer"
              >
                Batal
              </button>
