...
export const InventoryTable: React.FC<InventoryTableProps> = ({
  ...
  return (
    <div className="skeuo p-6">
      <form onSubmit={onSearchSubmit} className="flex gap-3 mb-6">
            ...
            {loading ? (
              ...
              materials.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    {item.is_low_stock ? (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-rose-50 text-rose-700 border border-rose-200/80 dark:bg-rose-950/50 dark:text-rose-300 dark:border-rose-800/60 inline-flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> Menipis
                      </span>
                    ) : (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200/80 dark:bg-emerald-950/50 dark:text-emerald-300 dark:border-emerald-800/60 inline-flex items-center gap-1">
                        Aman
                      </span>
                    )}
                  </td>
                  <td className="py-3 px-4 text-right">
                    <button 
                      ...
                      className="px-2.5 py-1.5 text-xs font-semibold skeuo-button text-blue-600 dark:text-blue-400 hover:text-blue-700 inline-flex items-center gap-1 rounded-lg"
                    >
                      <ArrowUpRight className="w-3.5 h-3.5" /> Mutasi Masuk
                    </button>
                  </td>
                </tr>
              ))
